...
export default function SubmitPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      const payload =
        tab === "paste" ? { code, language } : { repo_url: repoUrl };
      ...
      {tab === "paste" ? (
        <div className="mt-4">
          ...
          <div className="mt-1 flex justify-between text-xs">
            <span
              className={overLimit ? "text-danger" : "text-text-faint"}
              title="심사 정확도와 처리 시간을 위해 한 번에 한 파일, 500줄까지 심사합니다. 긴 파일은 문제가 있다고 의심되는 부분만 잘라 제출하면 판결이 더 정확합니다."
            >
