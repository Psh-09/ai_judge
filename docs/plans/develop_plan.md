...
화면 표시(사유 노출 등) 확인됨 — `/cases/[id]`의 FAILED 화면에서 실패 단계·사유 배너("심사에 실패했습니다 (PROSECUTION · API_ERROR)" 형태)가 실제로 노출되는 것을 스크린샷으로 확인했다.
...
마이페이지 화면에서의 표시 확인됨 — 전과 기록(`/cases`, 반복 조항 랭킹 포함)과 형량 체크리스트(별도 화면이 아니라 판결문 화면에 인라인, design.md 5.5 참고)가 실제로 렌더링되고 체크 토글이 반영되는 것을 스크린샷으로 확인했다.
...
- public 저장소의 파일 blob URL 제출 시 M1과 동일한 절차로 재판이 진행되고 API가 origin(`repo_url`/`commit_sha`/`file_path`)을 반환함
...
판결문 화면에서의 출처 표시 — 실제 공개 GitHub 파일(`https://github.com/Psh-09/ai_judge/blob/.../server/app/fixtures/sample_user_handler.py`)을 제출해 API가 `origin`(`repo_url`/`commit_sha`/`file_path`)을 올바르게 반환함을 확인했고, 프론트엔드 렌더링 코드(`client/app/cases/[id]/page.tsx`)도 이 값들을 조건 없이 그대로 표시하도록 되어 있음을 코드로 확인했다. 다만 브라우저 스크린샷으로 픽셀 단위까지 확인하지는 못했다(검증 도구 장애) — 다음 프론트엔드 작업 때 실제 화면으로 한 번 더 확인 권장.
