...
- 인증된 사용자 1인이 코드 제출부터 판결문 열람까지 실제 LLM 호출로 전 과정이 동작
- 스키마 검증 실패를 인위로 유도했을 때 재시도 후 `FAILED`로 종결되고 화면에 사유가 표시됨
...
- 사용자가 여러 건 제출한 뒤 마이페이지에서 반복 조항 통계가 실제 집계값으로 표시됨
